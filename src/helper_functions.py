...
def evaluate_model(
    ...
    plot_loglosses(
        num_epochs, loss_vals, l2_loss_vals, l2_lambda, validation_loss, reg_flag
    )
...
def plot_output_encoder_vs_decoder(model, datagen, alpha=0.1):
    ...
    assert (
        encoder_output.shape == decoder_output.shape
    ), "Encoder and Decoder outputs must be of the same size"
...
def run_lambda_variation(lambda_list, norm, epochs):
    ...
    for norm_lambda in lambda_list:
        ...
        model_training_params = train_decoder_w_hidden(
            model_decoder, loss_fn, l_norm=norm, n_epochs=epochs, n_lambda=norm_lambda
        )
...
def plot_corr_vs_epochs(epochs, corr_vals):
    ...
    """
    plt.plot(
        np.linspace(1, epochs, epochs).astype(int), corr_vals, label="$R^2$ coefficient"
    )
